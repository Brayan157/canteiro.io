...
8. A regra de inadimplência é:
   ...
   - as intenções de aviso são entregues por e-mail via `NotificationPort`, com adaptador SMTP isolado da assinatura. A entrega persiste estado, tentativas e auditoria `SYSTEM`; falhas são elegíveis para nova tentativa, e aviso cuja cobrança já foi confirmada ou cancelada é cancelado sem envio;
